...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for s in self.knowledge:
            if s == newsentence:
                continue
            elif s.cells.issuperset(newsentence.cells):
                setDiff = s.cells - newsentence.cells
                countDiff = s.count - newsentence.count
                newknowledge.append(Sentence(setDiff, countDiff))
            elif newsentence.cells.issuperset(s.cells):
                setDiff = newsentence.cells - s.cells
                countDiff = newsentence.count - s.count
                newknowledge.append(Sentence(setDiff, countDiff))
    ...
    def remove_sures(self):
        ...
        for s in self.knowledge:
            ...
            if s.known_mines():
                for mineFound in s.known_mines().copy():
                    self.mark_mine(mineFound)
                final_knowledge.pop(-1)
            elif s.known_safes():
                for safeFound in s.known_safes().copy():
